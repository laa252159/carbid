...
public class Mailer implements MailService {
    ...
    public void notifyAdminAboutPostConfirmation(User user) {
        notifyAdmins(SENDER, "На cars64.ru подтвердил почту пользователь " + user.getUsername(), "Подтвердил почту : \n\n" + user);
    }

    @Override
    public void notifyAdminAboutRegistration(User user) {
        notifyAdmins(SENDER, "На cars64.ru зарегистрировался пользователь " + user.getUsername(), "Зарегистрировался : \n\n" + user);
    }
    ...
    public void notifyUsersAboutNewAuction(Auction auction) {
        ...
        for (User user : users) {
            ...
            subject.append("Новый Аукцион на cars64: ");
            ...
            message.append("\nСсылка для незарегистрированных пользователей: http://auction.cars64.ru/auction/" + auction.getAuctionid());
...
//            notifyAdmins(SENDER, "ошибка при автоматической отправке письма для: " + to + " с текстом: ", msg); В один поток
    ...
    /**
     * Уведомление вызванное через submit формы (старый функциоал с cars64)
     */
    @Override
...
//            }
    ...
    @Override
     public void sendToUserMailConfirmationLink(User user) {
        StringBuilder subject = new StringBuilder();
        StringBuilder message = new StringBuilder();
        subject.append("cars64 Подтверждение электронной почты");
        message.append("cars64.ru сообщает, что Вы успешно зарегистрированы в системе. Для активизации Вашего " +
        ...
        message.append("http://auction.cars64.ru/approve-and-contract?email=" + user.getEmail() + " \n");
    ...
    public void sendToUserChangePasswordLink(User user, String token) {
        ...
        message.append("http://auction.cars64.ru/password_recovery?token=" + token + " \n");
    ...
    public void notifyUsersAboutVictory(Auction auction) {
        StringBuilder sb = new StringBuilder();
        sb.append(auction.getBuyer().getUsername() + ", поздравляем Вас! Вы выйграли аукцион http://auction.cars64.ru/auction/" + auction.getAuctionid() + "\n");
        ...
        sb2.append(auction.getBuyer().getUsername() + ", пользователь выйграл аукцион http://auction.cars64.ru/auction/" + auction.getAuctionid() + "\n");
